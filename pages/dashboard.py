...
with st.sidebar:
    ...
    selected_league = st.selectbox(
        ...
        options=["-- Select a League --"] + ACTIVE_LEAGUES
    )
...
def build_prompt(league):
    if league == "Tennis":
        return f"""
            ...
            ### LEAGUE: Tennis
            - In Season: Yes
            - Current Major Tournament: (Ongoing ATP/WTA/Grand Slam tournament, or "None")
            - Upcoming Big Tournaments: (Next 3 major ATP/WTA or Grand Slam tournaments with dates)
            - Current World Rankings: (Top 5 ATP and Top 5 WTA players as of today)
            - Injury News: (Only major injuries affecting top-ranked players; else say None)
            - Most Hyped Player Right Now + reason (based on recent performance or news)

            Rules:
            - Use live web search
            - All information must reflect CURRENT rankings and tournaments
            - If information is not available today, say "Not Available"
            - Be concise
            - Do NOT invent facts
            """
    else:
...
def fetch_dashboard():
    response = client.responses.create(
        ...
        input=build_prompt(selected_league),
    )
...
if selected_league != "-- Select a League --":
    if refresh or "dashboard_data" not in st.session_state or st.session_state.get("league") != selected_league:
        with st.spinner(f"Fetching live data for {selected_league}..."):
    ...
    st.info("👈 Select an active league from the sidebar to load data.")
